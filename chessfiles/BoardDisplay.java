...
public class BoardDisplay implements ActionListener
{
		...
		for (int row = 0; row < grid.length; row++)
			for (int col = 0; col < grid[row].length; col++)
			{
				...
				if (piece != null)
				{
					...
					icon = new ImageIcon(resolveImagePath(piece.getImageFileName()));
				}
	...
	private String resolveImagePath(String imageFileName)
	{
		...
		File chessFolderImage = new File("chessfiles", imageFileName);
		if (chessFolderImage.exists())
			return chessFolderImage.getPath();

		try
		{
			File classDir = new File(BoardDisplay.class.getProtectionDomain()
				.getCodeSource().getLocation().toURI());
			if (classDir.isFile())
				classDir = classDir.getParentFile();

			File[] candidates = {
				new File(classDir, imageFileName),
				new File(classDir, "chessfiles/" + imageFileName),
				classDir.getParentFile() == null ? null
					: new File(classDir.getParentFile(), "chessfiles/" + imageFileName)
			};
		...
		{
			// fall through to the original name if we cannot resolve a better path
		}
