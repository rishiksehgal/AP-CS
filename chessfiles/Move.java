...
public class Move
{
	...
	public boolean isCastleMove()
	{
		return piece instanceof King &&
			...
			Math.abs(source.getCol() - destination.getCol()) == 2;
	}

	public Location getCastleRookSource()
	{
		if (!isCastleMove())
			return null;

		int rookCol = destination.getCol() > source.getCol() ? 7 : 0;
		return new Location(source.getRow(), rookCol);
	}

	public Location getCastleRookDestination()
	{
		if (!isCastleMove())
			return null;

		int rookCol = destination.getCol() > source.getCol()
